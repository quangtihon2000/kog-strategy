"""Tier 2 Channel B parser tests — fixture-driven."""
...
def test_corpus_has_valid_and_invalid() -> None:
    valid = [f for f in CHANNEL_B_FIXTURES if f.expected is not None]
    invalid = [f for f in CHANNEL_B_FIXTURES if f.expected is None]
    assert len(valid) >= 12
    assert len(invalid) >= 8


def test_fixture_ids_unique() -> None:
    ids = [f.id for f in CHANNEL_B_FIXTURES]
    assert len(ids) == len(set(ids))


@pytest.mark.parametrize("fx", CHANNEL_B_FIXTURES, ids=lambda f: f.id)
def test_channel_b_parse(fx: ChannelBFixture) -> None:
