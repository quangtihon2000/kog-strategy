...
HELP_TEXT = (
    ...
    "/cancel\\_gvfx — cancel the current GVFX signal; pick scope via buttons:\n"
    "         block new entries (default) — or close all positions + pendings\n"
    "/zone — publish zone_signal; no args = wizard,\n"
    "         or `<upper>` `<lower>` `<targets_above>` `<targets_below>` `[symbol]`\n"
    "         targets are comma-separated price lists; symbol defaults to XAUUSD\n"
    "/cancel\\_zone — cancel the current zone signal; pick scope via buttons:\n"
    "         block new entries (default) — or close all positions + pendings\n"
    "/cancel — abort the GVFX or zone wizard mid-flow\n"
