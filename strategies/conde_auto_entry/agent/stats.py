...
def format_report(stats: Dict[str, ChannelStats], since_label: str) -> str:
    ...
    execution count, and the Wilson lower-bound on a single line. Channels
    with at least one execution append an indented second line with TP%
    and avg_R.
    """
    if not stats:
    ...
    lines = [
        ...
        "",
        f"{'channel':<{_NAME_W}} {'sig':>3} {'ex':>3} {'c95':>4}",
    ]
    for cs in rows:
        c95 = f"{cs.confidence_lo95:.2f}" if cs.n_executed > 0 else "-"
        lines.append(
            f"{_fmt_name(cs.channel)} {cs.n_signals:>3} {cs.n_executed:>3} {c95:>4}"
        )
        if cs.n_executed > 0:
            tp = f"TP {cs.tp_rate * 100:.0f}%" if cs.tp_rate is not None else "TP -"
            ar = f"R {cs.avg_r:+.2f}" if cs.avg_r is not None else "R -"
            lines.append(f"  {tp} · {ar}")
